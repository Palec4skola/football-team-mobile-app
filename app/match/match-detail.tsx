...
export default function MatchDetailScreen() {
  ...
  const openMatchLink = async () => {
    if (!match?.matchLink) return;

    const supported = await Linking.canOpenURL(match.matchLink);
    if (supported) {
      await Linking.openURL(match.matchLink);
    } else {
      console.warn("Neviem otvoriť tento link:", match.matchLink);
    }
  };
